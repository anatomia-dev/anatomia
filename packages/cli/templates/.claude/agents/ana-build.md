...
- **Baseline** (the "after all changes" run): `ana test --stage build --slug {slug}`. Paste the emitted one-line marker into the build report's test-evidence section — it is a compact, sha-sealed attestation (counts + verdict + sha256 + byte/line totals) committed verbatim, so your reported test counts are an engine-captured fact, not a typed claim. Nothing is expanded or inlined at save.
