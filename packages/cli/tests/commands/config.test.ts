...
describe('ana config', () => {
  ...
  describe('config set', () => {
    ...
    // @ana A017 — processCapture is a real schema field; setting it must not warn.
    it('does not warn when setting processCapture', async () => {
      await writeConfig(TEST_CONFIG);
      const program = await createProgram();
      await runCommand(program, ['config', 'set', 'processCapture', 'off']);
    ...
    it('sets mergeStrategy through config command', async () => {
      ...
      const program = await createProgram();
      process.exitCode = 0;
      await runCommand(program, ['config', 'set', 'mergeStrategy', 'squash']);
    ...
    // @ana A024, A025
    it('rejects invalid mergeStrategy through config command', async () => {
      await writeConfig({ ...TEST_CONFIG, mergeStrategy: 'merge' });
      const program = await createProgram();
      process.exitCode = 0;
      await runCommand(program, ['config', 'set', 'mergeStrategy', 'fast-forward']);

      expect(process.exitCode).toBe(1);
      const config = await readConfig();
      expect(config['mergeStrategy']).toBe('merge');
      const errorOutput = getErrorOutput();
      expect(errorOutput).toContain('Invalid mergeStrategy');
    });
  ...
  describe('mergeStrategy docs', () => {
    // @ana A030
    it('documents mergeStrategy recovery', async () => {
      const docsPath = path.resolve(originalCwd, '..', '..', 'website', 'content', 'docs', 'guides', 'troubleshooting.mdx');
      const content = await fs.readFile(docsPath, 'utf-8');
      expect(content).toContain('ana config set mergeStrategy');
    });

    // @ana A031
    it('documents mergeStrategy configuration', async () => {
      const docsPath = path.resolve(originalCwd, '..', '..', 'website', 'content', 'docs', 'guides', 'configurability.mdx');
      const content = await fs.readFile(docsPath, 'utf-8');
      expect(content).toContain('mergeStrategy');
    });
  });
});
