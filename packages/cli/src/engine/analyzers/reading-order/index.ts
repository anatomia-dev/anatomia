...
export interface ReadingOrderInput {
  /** The Slice-2 import graph (PageRank runs over `nodes`/`edges`). */
...
export function buildReadingOrder(input: ReadingOrderInput): ReadingOrder | null {
  ...
  // 1. Centrality — fixed-iteration PageRank, normalized to its own max so the
  //    weights stay comparable regardless of graph size.
  ...
  for (const file of graph.nodes) {
    const centrality = maxRank > 0 ? (ranks.get(file) ?? 0) / maxRank : 0;
    ...
    // Centrality reason last so risk/co-change (the actionable signals) read
    // first; always present because every graph node has a centrality score.
    ...
    entries.push({ file, score: Number(score.toFixed(6)), reasons });
  }
  ...
  const trimmed = trimToBudget(entries, budget);
