...
After reading the contract, run `ana proof context {files from contract file_changes}` to surface proof chain history for the modules this build touches. Let the findings inform what you pay attention to during code review. The **Shaped by** history — the verified work items that shaped each file — is **orientation only**: it tells you why a file is the way it is, so you know where to look. Consume the **Also changes with** co-change partners the same way — they show what historically moves with the files under review, widening where you look for regressions and missed updates. Both Shaped by and co-change are orientation; neither is a verdict. You form your findings **independently** from the spec, the contract, and the code in front of you — the two-account model only holds if your read is your own. If the build interacts with a known issue (addresses it, changes its impact, or works around it), note that in your findings. If your finding matches an active proof chain issue, reference it (e.g., "still present — see {finding-id}") rather than re-describing it. This counts toward the minimum finding requirement.
