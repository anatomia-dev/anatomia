...
```
[anatrace] unknown verdict reason "some-future-reason" from anatrace-core@<coreVersion> —
the engine may have drifted; recording verbatim. Update VERDICT_REASONS in src/types/proof.ts.
```

The engine version in the message is **interpolated from the resolved `coreVersion`**, never hardcoded — same one-version-literal discipline as AC4(ii). A future bump must not require editing this string.
...
**Constrain the diff:** After the install, run `git diff pnpm-lock.yaml` and confirm the churn is **limited to `anatrace-core` entries** — the 0.4.0 transitive deps are already present in the lockfile, so there should be **no new packages and no incidental version bumps to unrelated deps**. If `pnpm` opportunistically updated anything else, narrow it back (re-pin or revert the unrelated hunks) so the diff is the bump and nothing more.
**Why:** `--frozen-lockfile` in CI rejects any drift between `package.json` and the lockfile; a stale lockfile fails CI at install before a single test runs. A scoped lockfile diff also keeps the bump auditable — incidental dep updates riding this PR are exactly the silent scope creep the pipeline exists to catch.
...
- **Real-engine tests:**
  ...
  - **AC4(iii) — the load-bearing one:** build ONE in-class fixture and assert ≥1 verdict has `status === 'violated'` (assert on **status only — do NOT target a specific `reason`**). Use `installAgentDef('build')` as the mandate source; run it through the real engine via `captureComplianceAtSave`. Iterate the transcript bytes until it genuinely flips.
    - **The fixture MUST be the ANSI-C `$'...'`-decodable force-push class** (the #38 case): e.g. `git $'push' --force`, which 0.4.0 decodes and matches to the forbidden `git push --force` predicate → `violated` (0.2.0 off-by-one false-PASSed it).
    - **Do NOT use eval / expansion / pipe-to-shell / wrapper obfuscation.** That class returns `command-unresolvable` → `status: "unverifiable"` (verified in `anatrace-core@0.4.0 index.mjs:3160,3165`) — it is *genuinely* unresolvable, will NEVER flip to `violated`, and would trigger a **false STOP**. The fixture must be decodable-and-forbidden, not unresolvable.
    - **If no `$'...'`-decodable force-push fixture can be made to flip under 0.4.0, STOP and file a finding — do NOT swap in a trivially-violated command to go green.** A plainly-forbidden command any version catches proves nothing; a true STOP means the closure isn't reaching our emit path, which is the whole reason this item exists.
...
- **AC4(iii) fixture class — do not pick the wrong obfuscation (verified against engine bytes).** The fixture MUST be the ANSI-C `$'...'`-decodable force-push class (e.g. `git $'push' --force`): 0.4.0 decodes it and matches the forbidden `git push --force` predicate → `violated`. Do NOT use eval / expansion / pipe-to-shell / wrapper obfuscation — that returns `command-unresolvable` → `status: "unverifiable"` (verified `anatrace-core@0.4.0 index.mjs:3160,3165`), never flips to `violated`, and triggers a **false STOP**. Assert `status === 'violated'` only; never target a specific `reason`.
...
- After the install: `git diff --stat pnpm-lock.yaml` then `git diff pnpm-lock.yaml` — expected: only `anatrace-core` entries changed; no new packages, no unrelated dep bumps. Narrow any incidental hunks.
