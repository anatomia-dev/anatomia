...
- AC3: `findStackProvenance` checks `database`, `auth`, `payments`, and `aiSdk`. It does NOT check `testing` (testing detection intentionally uses rootDevDeps fallback — provenance is expected to be cross-package) or `framework` (framework comes from config files and shape detection, not allDeps).
