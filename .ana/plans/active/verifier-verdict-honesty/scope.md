...
- `dist/templates/*` is auto-generated by the build (`tsup && cp -r templates dist/`) — never hand-edit it;
  edit the `templates/` masters and rebuild. Dogfood (`.claude`/`.codex` at repo root) is a manual copy,
  enforced byte-equal by `agent-proof-context.test.ts` — but see the codex-guard gap below.
...
- **(Codex-guard gap — half-enforced lockstep)** `agent-proof-context.test.ts:67-74` asserts dogfood
  byte-identity for the **`.claude`** agents dir only (`../../../../.claude/agents`); no `.codex` equivalent was
  found in it. Confirm whether the codex dogfood copy is guarded anywhere. If it is **not**, the codex
  `ana-verify.md` can silently drift from its template — a direct violation of the "must work for both
  platforms, lockstep" constraint. If unguarded, fold "add a `.codex` dogfood byte-identity assertion" into
  Component 1 so the prompt-contradiction fix is enforced on both harnesses, not just claude.
