...
- `packages/cli/src/types/proof.ts` (modified): **Touch 1/4** — added optional `process?: ProcessAttestation` to `ProofChainEntry` and defined the `ProcessAttestation` interface (session identity + `derived` + `outcome` + `task_shape` + `module_churn`).
...
- `packages/cli/src/commands/work-proof.ts` (modified): **Touch 3/4** — reads `module_churn` from `.saves.json`; assembles and spreads the optional `process` attestation at work-complete, gated on `isProcessCaptureEnabled` + a matching worktree session. Exported `assembleProcessAttestation`. **Contains the human-approved slug-recovery DEVIATION (see Deviations).**
- `packages/cli/src/commands/proof.ts` (modified): **Touch 4/4** — display-only "Provenance" section (harness · role · model, turns/tool-calls, tokens, est. cost + table version, churn), gated on `entry.process`. Never influences PASS/FAIL. Added `formatTokenCount`.
...
- Attaches an optional `process` attestation to the proof at `ana work complete` via the `commit_hygiene` 4-touch pattern: outcome joins, task shape, and per-file `module_churn`. The field is decoupled and never gates the proof — capture off or no matching session → a valid proof that simply omits it.
...
- AC9 "ProcessAttestation attach via 4-touch" → `work-proof-process.test.ts` "attaches an attestation…", "joins outcome and task_shape…" (A031), first_pass_verify (A032), capture-off/no-match → null (A033)
...
- **Newest-timestamp record selection** for the attestation (deterministic; documented in Open Issues as it can pick the verify session over build).
...
### After Changes
Command: `ana test --stage build --slug session-capture` → `✓ captured  counts: 3520 passed, 0 failed, 2 skipped  (verdict: pass)`

Sealed capture marker (live seal — intentionally outside any code fence):

<!-- ana:capture stage=build slug=session-capture counts=3520p/0f/2s verdict=pass sha256=78985de50da47e13159b60c43f4f4ffe363d50e26a081b61431e8111ef42b599 -->

Tests: 3520 passed, 0 failed, 2 skipped

Full project-wide (`pnpm run test -- --run`): cli **145 files, 3520 passed, 2 skipped, 0 failed**. (Console `Error:`/`Push failed` lines in stdout are output from tests exercising the learn/strengthen error path with mocked git — not test failures; the suite reports 0 failed.)

### Comparison
- Tests added: 51 (3520 − 3469)
- Tests removed: 0
- Regressions: none
...
- `tests/commands/work-proof-process.test.ts`: attach (A031), first_pass_verify (A032), outcome/task_shape joins, the slug-recovery deviation, capture-off/no-match/dangling → null (A033).
...
4. **Attestation role label** (`work-proof.ts`, observation/acknowledge): newest-timestamp selection can pick the verify session over build, so `process.role` may read `verify`. Provenance is still correct; documented as a deliberate deterministic choice.
