...
- **Kind:** chore
- **Size:** small — version string updates across ~10 locations, one branch protection API call
- **Files affected:** `package.json`, `packages/cli/package.json`, `packages/cli/tsup.config.ts`, `.github/workflows/test.yml`, `README.md`, `website/content/docs/start.mdx`, `website/content/docs/guides/troubleshooting.mdx`, `website/public/llms-full.txt`
...
- AC9: `website/public/llms-full.txt` updated (2 occurrences of "Node 20" → "Node 22")
- AC10: All tests pass on both Node 22 and Node 24
...
### Relevant Code Paths
- `package.json:23` — root engines field: `"node": ">=20.0.0"`
- `packages/cli/package.json:40` — package engines field: `"node": ">=20.0.0"`
- `packages/cli/tsup.config.ts:6` — build target: `target: 'node20'`
- `.github/workflows/test.yml:24` — matrix: `node-version: [20, 22]` (will be `[20, 22]` after scope 1, needs to become `[22, 24]`)
- `.github/workflows/test.yml:67` — coverage gate: `matrix.node-version == 20`
...
- `website/public/llms-full.txt:710,1052` — two "Node 20" mentions
...
### Known Gotchas
- `llms-full.txt` is a generated/compiled file — check whether it's manually maintained or built from the mdx content. If built, update the source and regenerate. If manual, update directly.
...
- Whether `llms-full.txt` is hand-maintained or generated from the mdx docs — determines whether to edit it directly or rebuild it
