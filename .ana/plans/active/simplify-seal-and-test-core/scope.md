...
- **The final verify seal** has *no* downstream re-run and runs full-project (abstains, no per-test data), so it is hand-forgeable today. This is the one place nothing stands behind the seal — the residual integrity hole, closable only by an engine-bound token (`enginebind` / L3), which is the named follow-on (see "Follow-on" below).
...
- **The hash is recomputable — by design (see Approach + Rejected Approaches).** The seal is hand-recomputable from its visible fields: determinism, not anti-forgery. Forgery resistance is NOT the gate (presence/well-formedness only); it is **Verify's independent re-run for the build seal**, and — for the **final verify seal**, which has no re-run and abstains — **nothing yet**, pending `enginebind` (the named follow-on). Surfaced so the integrity story is honest and the residual hole is tracked, not implied-closed.
...
## Follow-on (tracked, not someday): engine-bound verify seal

This scope deliberately leaves the **final verify seal hand-forgeable** — and the per-test-digest alternative would have left it forgeable too, because the verify run abstains (no per-test data to bind). The build seal is backstopped by Verify's independent re-run; the **verify seal has nothing behind it** — it is the one place in the pipeline where nothing stands behind the seal.

Closing it requires an **engine-bound token** (the reserved `enginebind` field / L3): a value the engine emits that cannot be reproduced by hand-typing the marker. That is the **next real integrity scope** — named here so it is carried forward as the direct successor, not left as a someday. It is explicitly OUT of scope for this subtraction work (which must not grow new machinery), but accepting summary-only (option (b)) *is* the decision to make `enginebind` the next integrity scope.

> **AnaPlan/founder note:** when this scope completes, the engine-bound verify seal should be opened as the following work item. The `enginebind` field is left intact by this scope precisely to be its landing point — do not remove it.
...
- **Engine-bound verify seal named as the tracked follow-on** (new "Follow-on" section): summary-only makes `enginebind`/L3 the explicit next integrity scope, not a someday. `enginebind` is left intact as its landing point.
