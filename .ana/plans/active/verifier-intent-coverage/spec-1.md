...
- `joinCoverage(scopeContent: string, contract: ContractSchema): CoverageJoin` — **a standalone exported helper, built from the start, that the gate calls internally.** It runs `extractScopeACs`, then for each AC id resolves its coverage status by joining against every assertion's `ac:` field and the `coverage_waivers` entries. Returns a per-AC join (define and export the `CoverageJoin` interface: at minimum `{ acs: Array<{ id: string; status: 'pinned' | 'judgment' | 'retired' | 'uncovered'; assertions: string[]; weakOnly: boolean }>; ambiguous: boolean }`). **This is exported deliberately so Phase 2 (proof coverage computation + `ana plan coverage`) consumes the same join — no Phase-2 refactor of Phase-1 code across a PR boundary. Foundation, not refactor-later.**
- `evaluateCoverageGate(input: { scopeContent: string; contract: ContractSchema }): CoverageGateResult` — the pure gate, a thin policy layer over `joinCoverage`. Define and export the `CoverageGateResult` interface (shape in Output Mockups). Activation logic: parse `contract.version` (major.minor compare) → `active` only when `version >= "1.1"` AND the join is non-ambiguous AND it has ≥1 AC. When active, `uncovered` = the join's `uncovered`-status AC ids. `block = active && uncovered.length > 0`. `info` carries the join's `weakOnly` ACs. Always populate `diagnostic`. Never throw — a malformed contract/scope degrades to a warn-only result.
...
**What changes:** The AC1 live-corpus measurement, built on an **oracle independent of the extractor's own classifier** — otherwise the test grades the extractor against itself. **The oracle is the literal `## Acceptance Criteria` heading in the scope file** (a fact about the file, not a product of `extractScopeACs`). Glob `<repoRoot>/.ana/plans/completed/*/scope.md` (resolve repo root explicitly — see Gotchas). For every scope whose raw text contains a literal `## Acceptance Criteria` heading, assert:
- `extractScopeACs(content).ids.length > 0` — it recovered at least one id (`emptyExtractionCount` = count of oracle-positive scopes that returned an empty id set; assert `toBe(0)`).
- `extractScopeACs(content).ambiguous === false` — it did not misclassify a scope the oracle says has an AC section (`falseAmbiguousCount` = count of oracle-positive scopes flagged ambiguous; assert `toBe(0)`).
- The extractor never throws on any corpus scope.

Do NOT decide "well-formed" from the extractor's output — the heading decides it; the id-parsing is what is under test. Also assert the glob found **> 50 scopes** (catch a vacuous `cwd`-relative pass), guarded for the legitimately-empty-corpus case (a stranger project with zero completed plans → skip the count assertion, never fail). Assert exact counts (`toBe(0)`), per testing-standards. `corpusMeasurement.falseAmbiguousCount` / `corpusMeasurement.emptyExtractionCount` (the contract's A006/A007 targets) are exactly these two oracle-computed counts.
...
- [ ] **AC1:** `extractScopeACs`, run over the live completed-scope corpus (glob, ~204 scopes) with the `## Acceptance Criteria` heading as the independent oracle, recovers a non-empty AC id-set and misclassifies **zero** oracle-positive scopes as `ambiguous`. The "well-formed" judgment comes from the heading, never from the extractor's own output (no self-grading). Handles `- AC1:`, `## AC1`, `**AC1**`, `AC1:`. **This is the gate on the gate** — must be green before `evaluateCoverageGate` is wired to block (OQ3: degrade to warn if it can't pass).
...
- **Corpus (`scope-ac-corpus.test.ts`):** the AC1 sweep, oracle-driven. The `## Acceptance Criteria` heading (a file fact, independent of the extractor) selects the scopes under test; exact `toBe(0)` on false-ambiguous and empty-extraction counts among oracle-positive scopes; `> 50` scopes found (vacuous-pass guard), guarded for an empty corpus.
