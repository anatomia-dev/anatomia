...
- [Claude Code](https://claude.ai/code) or Codex — needed for pipeline agents (Steps 3–4)
- [GitHub CLI](https://cli.github.com/) (`gh`) — used by Verify to create PRs after a passing run

<ForPlatform platform="claude-code">
Claude Code users need the `claude` executable available on PATH before running pipeline agents. If you use stricter permission prompts, configure Claude Code flags after init.
</ForPlatform>

<ForPlatform platform="codex">
Codex users need the `codex` executable available on PATH before running pipeline agents. Codex projects use generated `.codex/agents/*.agent.toml` manifests for agent delivery.
</ForPlatform>
