...
The scan assigns your project one application shape from a fixed set: `web-app`, `api-server`, `full-stack`, `cli`, `library`, `mobile-app`, `worker`, `mcp-server`, or `ai-agent` — or `unknown` when nothing matches. The shape is detected once, from the signals of your primary package; it is not derived by counting surfaces. Whether a project is a monorepo is a separate fact the scan records on its own.

Shape is determined by a priority chain in which the most specific signal wins, and framework evidence outranks dependency and entry-point signals. A project with a browser UI framework like Next.js is a `web-app`, not "a Node project with React dependencies." A project with both a CLI entry point and a React frontend is a `web-app` — the framework outranks the `bin` field — not `cli`. `full-stack` is reserved for a server framework combined with browser-UI dependencies.
