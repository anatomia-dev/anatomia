...
<p style={{fontSize:"13px",color:"var(--ink-60)",margin:"0 0 16px"}}>User fields (`branchPrefix`, `coAuthor`, `artifactBranch`, `mergeStrategy`, `commands`, `custom`) are yours. Machine fields (`anaVersion`, `lastScanAt`, `language`, `framework`) refresh on `ana init`. Unknown top-level keys you add are preserved across re-init.</p>
...
The six agent files in platform-specific directories such as `.claude/agents/` and `.codex/agents/` are yours to edit. Adjust Verify's disposition, add constraints to Build, modify Think's scoping process. Copied on `ana init`, not symlinked — edits persist across re-init.
...
<li>Agent files in platform directories such as .claude/agents/ and .codex/agents/</li>
