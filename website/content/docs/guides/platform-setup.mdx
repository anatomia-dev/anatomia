...
Build and Verify need filesystem access to the worktree so they can edit files, run tests, and write reports. The generated configuration uses `danger-full-access` sandbox mode for pipeline agents because restricted sandboxes cannot complete those stages reliably.
