...
Anatomia scans the full repository, not individual packages. Init detects your primary package (largest `apps/` package with framework hints) and focuses convention detection there. Per-package scanning is planned. `commands.test` and `commands.build` are project-wide by default. To scope pipeline work to a specific package, set `commands.testPackage` for the primary package focus, or use Build Brief checkpoint commands for per-scope targeting.
...
1. Check the detected commands: `ana config show` — look at `commands.test` (project-wide) and `commands.testPackage` (primary package)
2. In monorepos, init may target the wrong package or produce an invalid invocation
3. Override: `ana config set commands.test "your-command"` for project-wide, or `ana config set commands.testPackage "your-command"` for the primary package
