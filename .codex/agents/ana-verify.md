...
**Run your independent test re-run through `ana test --stage verify --slug {slug}`** so Verify's run is captured and sha-sealed into `verify_report.md` — two independent sealed accounts of the same work. For the focused command, use `ana test --slug {slug} -- {checkpoint command from the Build Brief}`. (`verify_report.md` itself is never gated — only `build_report.md` is.)
